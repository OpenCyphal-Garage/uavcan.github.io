...
The list of core design goals below help explain the basic UAVCAN concepts and the motivation behind them.

* **Democratic network** - All nodes in the network should have the same communication rights so that there is no single point of failure.
* **Nodes can exchange long payloads** - Nodes should be able to transfer large sets of related parameters (e.g., GNSS solution, 3D vectors, etc.) in a single frame. This avoids the computational and programming cost required to split messages into separate frames and then reassemble them at receiving nodes.

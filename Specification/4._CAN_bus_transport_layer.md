...
CAN bus does not allow different nodes to transmit CAN frames with different payload under the same CAN ID.
...
The source of the random data must be likely to produce different discriminator values for different payloads.
...
with the same CAN ID but different payloads will be higher than 0.006%, which is significant.
...
Payload field of the CAN frame is shared between the following fields:

Field                   | Description
------------------------|----------------------------------------------------------------------------------------------
Transfer payload        | Actual payload of the transfer
Tail byte               | The last byte of the CAN frame payload field that contains auxiliary fields of the transport layer
...
The following figure summarizes the payload format for a single-frame transfer:

<a href="figures/single_frame_transfer.png">
<img src="figures/single_frame_transfer.png" style="max-width: 100%"/>
</a>

The following figure summarizes the payload format for a multi-frame transfer:
...
### From redundant interfaces

### From a non-redundant interface

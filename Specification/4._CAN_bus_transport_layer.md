...
Since the discriminator is only 14 bits long, the probablility of having multiple nodes that are emitting CAN frames
...
transfer desciptors to receiver states, which will be referred to as *receiver map*.
...
requirements (e.g., maximum recovery time in an event of an interface fialure), or the protocol stack
...
If the matching state does not exist, the node will add a new unintialized receiver state to the map.
The node then proceeds with the procedure of *receiver state update*, which will be defined below.
...
important information concerning CAN bus bit rate, connectors, and other properties concerning the physical layer
of the protocol.

A note should be made concerning implementation of the CAN controller driver software.
...
ensure correct behavior, and possibly apply work-arounds if necessary.

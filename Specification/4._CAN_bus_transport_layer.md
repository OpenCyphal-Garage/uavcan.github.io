...
Numerically lower priority values indicate higher urgency, and numerically higher values indicate lower urgency.
...
A broadcasted message is carried by a single message transfer that contains the serialized message.
...
Anonymous message is a message that originates from a node that does not have a node ID.
...
therefore it is advised to avoid the latter when appropriate.
...
<img src="/Specification/figures/transfer_crc_input.png" style="max-width: 100%"/>
...
An example of such special case is the time synchronization algorithm leveraged by UAVCAN
(it is documented in the following sections of the specification).
...
<a href="/Specification/figures/can_id_format.png">
<img src="/Specification/figures/can_id_format.png" style="max-width: 100%"/>
</a>
...
As can be seen from the description, valid values for priority range from 0 to 31, inclusively,
where 0 corresponds to highest priority, and 31 corresponds to lowest priority.

#### Message type ID

As can be seen from the description, valid values of message type ID range from 0 to 65535, inclusively.

Message type ID values that are allowed for use in anonymous message transfers are restricted to the range from 0 to 3,
inclusively.
This limitation is due to the fact that only 2 lower bits of the message type ID are available in this case.

#### Service type ID

As can be seen from the description, valid values of service type ID range from 0 to 255, inclusively.

#### Node ID

Node ID is represented by a 7-bit unsigned integer value.
Zero is reserved to represent an unknown node, or all nodes, depending on the context.
Therefore, node ID values are restricted to the range from 1 to 127, inclusively.
...
However, anonymous message transfers violate it, since they all share the same node ID of zero.

In order to work-around the problem, UAVCAN adds a discriminator to the CAN ID field of anonymous message
...
Nodes that are using this approach will be using the same discriminator value for identical messages,
...
with the same CAN ID but different data will be higher than 0.006%, which is significant.
...
detected.
This measure allows to prevent the bus deadlock that may happen if the automatic retransmission on bus error
is not suppressed.

### Payload

<a href="/Specification/figures/can_payload_format.png">
<img src="/Specification/figures/can_payload_format.png" style="max-width: 100%"/>
</a>

Data field of the CAN frame is shared between the following fields:

Field                   | Description
------------------------|----------------------------------------------------------------------------------------------
Transfer payload        | Actual payload of the transfer
Tail byte               | The last byte of the CAN frame data field that contains auxiliary fields of the transport layer
...
<a href="/Specification/figures/single_frame_transfer.png">
<img src="/Specification/figures/single_frame_transfer.png" style="max-width: 100%"/>
</a>

The following figure summarizes the data field format for a multi-frame transfer:

<a href="/Specification/figures/multi_frame_transfer.png">
<img src="/Specification/figures/multi_frame_transfer.png" style="max-width: 100%"/>
</a>
...
For multi-frame transfers, this field contains the value of the toggle bit, which is specified above.

#### Transfer ID

For all kinds of transfers, this field contains the transfer ID value of the current transfer.
...
The following explanation will rely on the concept of *transfer ID forward distance*.
...
A node that is subject to receiving a transfer must keep a certain set of state variables per transfer descriptor.
...
transfer descriptors to receiver states, which will be referred to as *receiver map*.
...
Transfer timestamp              | The local time when the first frame of the transfer has arrived
...
* Uninitialized - this is a default condition, which indicates that the receiver state has not seen any transfer yet,
and therefore it is considered uninitialized.
* Transfer ID timeout - last matching transfer has been seen more than 2 seconds ago.
...
A note should be made about implementation of the CAN controller driver software.
...
ensure correct behavior, and possibly apply workarounds if necessary.

...
Old allocations can be removed in order to free node ID for new allocations by means of clearing the Raft log
on all allocators.
...
Run-time cluster membership changes are not supported, since they are not needed for a vehicle bus.
...
An allocator is allowed to send follow-up allocation responses only if both conditions are met:
...
Such a situation can occur as a result of a very unlikely failure of redundant interfaces.
...
as in this case no follow-up responses are necessary.
So only CAN FD can offer perfectly reliable allocation exchanges.

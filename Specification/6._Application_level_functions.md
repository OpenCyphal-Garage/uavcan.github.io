...
Leader's node ID                | 1
Allocatee's unique ID           | `44 C0 8B 63 5E 5 F4 BC 83 3B 3A 88 1C 43 60 50`
Preferred node ID               | 0 (any)
Allocated node ID               | 125
...
it published a cluster discovery message in order to become aware of its siblings,
...
The allocators have detected the fact that all nodes in the cluster are now aware of each other,
...
The allocatee have appeared on the bus and published first-stage and second-stage allocation requests.
...
the leader performed a keep-alive AppendEntries call to the allocator 3:
...
Then the allocatee broadcasted the third-stage allocation request:

    2.871  1E63ED00   00 1C 43 60 50 C2

At this point the leader has received the full unique ID of the allocatee,
and started the process of allocation and log replication.
...
(it cannot complete the allocation until the new log entry is committed),
the allocatee has given up waiting for allocation and decided to restart the process.
...
While the leader was engaged with communications with the allocatee,

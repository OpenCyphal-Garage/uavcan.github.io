...
The points concerning hardware design practices are listed in this chapter.
Following them is highly recommended, because that would ensure the highest level of inter-vendor compatibility
and would allow the developers to avoid some of the common pitfalls.
...
The UAVCAN standard defines several connector types targeted for different application domains:
from highly compact systems to large deployments, from low-cost to safety-critical applications.
It is extremely undesirable to deviate from the recommendations provided in this section,
because that could hinder the cross-vendor compatibility.
...
**<span style="white-space: nowrap;">UAVCAN Micro</span>**  |JST JH 4-circuit               |5 V, 1 A           |Dronecode Autopilot Connector Standard
...
The UAVCAN D-Sub connector type is based upon and compatible with the most popular,
...
This type is a popular industry-standard connector, and there are many vendors that manufacture
...
M8 is the recommended connector type for all UAVCAN applications.
...
* M8 connectors might be a poor fit for applications that have severe weight and space constraints.
...
Devices are equipped with the male plug connector type (see the image) mounted on the panel or on the PCB,
and the cables are equipped with the female socket connectors on both ends.
Please beware of the risk of confusion between A-coded and B-coded M8 connectors,
as they are not mutually compatible.
...
The provided pinout, as has been indicated above, is compatible with the CiA 103 specification (CANopen).
...
The UAVCAN Micro connector is compatible with the Dronecode Autopilot Connector Standard.
...
UAVCAN permits to keep one of the redundant interfaces unused.
...
which often creates a situation when multiple devices with a different number of redundant interfaces need to be
...
It is encouraged to implement CAN auto bit rate detection when applicable.
...
Please refer to the [chapter dedicated to the standard data types](/Specification/6._Application_level_functions)
for details.

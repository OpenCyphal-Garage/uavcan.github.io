...
int main(int argc, const char** argv)
{
    ...
    /**
     * The most common way to configure UAVCAN hardware acceptance filters is just to use configureCanAcceptanceFilters
     ...
     * acceptance filters had been configured.
     */
    ...
    */
    ...
    /**
     * In case you need to create custom hardware acceptance filters or you want to output your computed configurations
     * the following approach must be used.
     *
     * The first step to configure UAVCAN hardware acceptance filters is to create the object
     * uavcan::CanAcceptanceFilterConfigurator with the desired node as an input argument.
     *
     * PLEASE, NOTICE! Only for this tutorial we are going to put the second argument, which override the actual number
     * of available hardware filters to 6. You may never use this feature and it's only made to make this tutorial more
     * illustrative.
     * There is also MaxCanAcceptanceFilters parameter in libuavcan/include/uavcan/build_config.hpp that limits the
     * number of maximum CAN acceptance filters available on the platform. You may increase this number manually if your
     * platform has higher number.
     *
    ...
    /**
     * The method computeConfiguration() gather information from all subscribers and service messages on the
     * configurator's node. Moreover, it creates a container with automatically calculated configurations of Masks and
     * ID's and subsequently apply the configurations to CAN controller.
     *
     * It may or may not take an argument:
     * - IgnoreAnonymousMessages
     * - AcceptAnonymousMessages (default, if no input arguments)
     *
     * By default filter configurator accepts all anonymous messages. But if you don't need it you may specify the
    ...
    /**
     * Our node keep getting air_data::Sideslip messages since configureCanAcceptanceFilters() was called previously and
     * service message file::BeginFirmwareUpdate since the service was declared (always accepted by UAVCAN, again). But
     * the air_data::TrueAirspeed is still not being accepted, even though it's within the multiset_configs_ container.
     * In order to carry out the configuration let's call applyConfiguration() in 3 seconds and start to get another
     * message.
     */
    node.spin(uavcan::MonotonicDuration::fromMSec(3000)); // Wait 3 seconds
     ...
     * called, otherwise you might loose your custom configurations. Each time computeConfiguration() is invoked, it
     ...
     * the number of configurations will reduce to 6.
     */
    anon_test_configuration.applyConfiguration();
